...
class MyAI(object):
    ...
    def get_next_step(self, chessboard: ChessBoard):
        ...
        该函数应当返回四个值: 
            ...
            4 落子的纵坐标
        '''
        self.a_b_cut(1,-999999,999999,chessboard)
        
        return self.past+self.now
    @staticmethod
    ...
    def a_b_cut(self, depth, a, b, chessboard: ChessBoard):
        if depth>=self.max_depth:
            ...
            for chess in chess_list:
                ...
                if depth%2==1 and chess.team ==self.team:
                    ...
                    for new_x,new_y in next:
                        ...
                        #走到下一步
                        chessboard.chessboard_map[new_x][new_y]=chessboard.chessboard_map[old_x][old_y]
                        #更新图片
                        ...
                        #复原当前棋局
                        chessboard.chessboard_map[old_x][old_y]=chessboard.chessboard_map[new_x][new_y]
                        chessboard.chessboard_map[old_x][old_y].update_position(old_x,old_y)
                        chessboard.chessboard_map[new_x][new_y]=origin_chess

                            #该点的权值大于当前a值，则意味着有更好的走法
                            #或者之前没有记录过最佳移动步骤，防止在深度搜索的过程中多次记录相同的最佳步骤
                            #如果是第一层,就记录下最好的走法
            ...
            if depth%2==1:
                ...
                return b
            #alpha-beta剪枝算法其实就是深度优先搜索过程中维护了两个值，不断更新并且进行判断，最终找到最优路径。因此在编写程序时可以进行递归的操作
            #而在本项目中需要将棋子移动到新的位置，更新棋子的位置信息，再将原来位置上的棋子暂时保存起来。
            #递归调用 self.a_b_cut() 方法，继续搜索下一层。恢复原来的棋局状态，以便进行下一次搜索。
            #在理论实践和实验的实践中可以发现实际就是访问到叶子节点后不断向上更新alpha&beta值，最终使得更快速找到一条minmax路径