class Predicate:
    def __init__(self, str_in):
        self.elements = []  # 每个谓词对象都有自己的 elements 列表
        if len(str_in) != 0:
            if str_in[0] == ',':
                str_in = str_in[1:]
            tmp = ""
            for i in range(len(str_in)):
                tmp += str_in[i]
                if str_in[i] in ['(', ',', ')']:
                    self.elements.append(tmp[0:-1])  # 将谓词元素添加到对象的 elements 列表中
                    tmp = ""
...
def end_or_not(new_clause, set_of_clause):
    ...
    if len(new_clause) == 1:  
        for i in range(len(set_of_clause) - 1):  
            if len(set_of_clause[i]) == 1 and new_clause[0].get_name() == set_of_clause[i][0].get_name() and new_clause[0].elements[1:] == set_of_clause[i][0].elements[1:] and new_clause[0].get_pre() != set_of_clause[i][0].get_pre():
        ...
        for i in range(len(set_of_clause)):
            ...
            if len(set_of_clause[i]) == 1:
                for j in range(len(set_of_clause)):
                    ...
                    if end_or_not(new_clause, set_of_clause):  # 使用新函数判断是否结束
                        status = False
                        break
            else:
                for j in range(len(set_of_clause)):
